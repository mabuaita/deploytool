...
BUCKET_POSTFIX = ''  # Gets put after the unix user ID to create the bucket name.
SSH_KEY_DIR = os.environ['HOME'] + '/.ssh'
SQS_QUEUE_NAME = APP_NAME + 'Queue'
LAMBDA_FUNCTION_NAME = 'ecs-worker-launcher-sym'
LAMBDA_FUNCTION_DEPENDENCIES = 'async'
ECS_TASK_NAME = APP_NAME + 'Task'

# Constants (OS specific)
BUCKET = 'iqcaaa'
AWS_BUCKET = BUCKET + BUCKET_POSTFIX
...
LAMBDA_FUNCTION_CONFIG = {
    "s3_key_suffix_whitelist": ['/aaa'],  # Only S3 keys with this URL will be accepted.
...
def get_or_create_bucket():
	...
	print('Did Not Find bucket: ' + AWS_BUCKET + '.')
	sys.exit()
...
def update_ecs_image():
    ...
    with cd('~/' + APP_NAME):
        ...
        login_str = local('aws ecr get-login | sed -e "s/-e none//g"', capture=True)
        print(login_str)
        run('%s' % login_str)
